...
class AES(object):
    ...
    def cipher(self, expanded_key, data):
        ...
        decimal_list=state
        hex_list = ' '.join([hex(num)[2:].zfill(2) for num in decimal_list])
        print("Round 0 : ",hex_list)
        self.round_outputs.append(hex_list)
        ...
        for r in range(self.Nr - 1):
            ...
            decimal_list2=state
            hex_list2 = ' '.join([hex(num)[2:].zfill(2) for num in decimal_list2])
            print("Round",r+1,": ",hex_list2)
            self.round_outputs.append(hex_list2)

        state = self.sub_bytes(state)
        state = self.shift_rows(state)
        state = self.add_round_key(state, expanded_key[self.Nr])
        decimal_list3=state
        hex_list3 = ' '.join([hex(num)[2:].zfill(2) for num in decimal_list3])
        print("Round 14 : ",hex_list3)
        self.round_outputs.append(hex_list3)
...
if __name__ == "__main__":
        ...
        aes2 = AES(key)
        cyphertext2 = aes2.encrypt(data2)
        print("Cipher Text: ",cyphertext2)

        print("\n\n------------------------Avalanche Effect--------------------------------\n")    
        for i in range(14):  # Assuming there are 10 rounds
             round_output_A = aes.round_outputs[i].replace(" ", "")  # Remove spaces
             round_output_B = aes2.round_outputs[i].replace(" ", "")  # Remove spaces
        ...
        data3 = "Hello Worls!"
        print("Encryption ")
        print("Plaintext: ",data3)
        aes3 = AES(key)
        cyphertext3 = aes3.encrypt(data3)
        print("Cipher Text: ",cyphertext3)
        
        
        print("\n\n------------------------Avalanche Effect--------------------------------\n")    
        for i in range(14):  # Assuming there are 10 rounds
            round_output_A3 = aes.round_outputs[i].replace(" ", "")  # Remove spaces
            round_output_B3 = aes3.round_outputs[i].replace(" ", "")  # Remove spaces
