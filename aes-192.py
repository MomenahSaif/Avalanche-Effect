import textwrap
import numpy as np
class AesFromScratch:
    sBox = list()
    GaloisF2 = list()
    GaloisF3 = list()
    RCON = list()
    

    def __init__(self):
        self.initTables()
        self.round_outputs = []
    # Initialize Tables
    def initTables(self):
        # AES S-Box
        self.sBox = [
            
        ]

        # GF 2^8 {2}
        self.GaloisF2 = [
            
        ]

        # GF 2^8 {3}
        self.GaloisF3 = [
            
        ]

        # RCON table for up to ten rounds
        self.RCON = ["01", "02", "04", "08", "10", "20", "40", "80", "1b", "36"]

    # Initialize state matrix
    def initSteMat(self, plaintext):
        state = []

        while plaintext:
            state.append(plaintext[:2])
            plaintext = plaintext[2:]

        return state

    # Opertaion Functions
    def xor(self, left, right):

        out = int('0x{}'.format(left), 0) ^ int('0x{}'.format(right), 0)
        out = str(hex(out))[2:]

        if len(out) < 2:
            out = "0" + out

        return out

    def rotateWord(self, word):
        return word[1:] + word[:1]



    # Key Schedule 1st part
    def SboxMatch(self, word, round):
        output = []
        outString = ''

        while word:
            output.append(word[0:2])
            word = word[2:]

        output = self.rotateWord(output)

        for index in range(len(output)):
            out = int("0x{}".format(output[index]), 0)
            out = str(hex(self.sBox[out]))[2:]

            if len(out) < 2:
                out = "0" + out

            output[index] = out

        output[0] = self.xor(output[0], self.RCON[round])

        for char in output:
            outString += char

        return outString


    def keySchedule(self, key, round):
        words = []
        roundKey = [None, None, None, None]

        while key:
            words.append(key[:8])
            key = key[8:]

        if round % 6 == 0:
            # Rotate, SubWord, XOR with RCON
            roundKey[0] = self.xor(self.SboxMatch(self.rotateWord(words[3]), round // 6), self.RCON[round // 6]).zfill(8)
            roundKey[1] = self.xor(words[0], roundKey[0]).zfill(8)
            roundKey[2] = self.xor(words[1], roundKey[1]).zfill(8)
            roundKey[3] = self.xor(words[2], roundKey[2]).zfill(8)
        else:
            roundKey[0] = self.xor(words[0], self.SboxMatch(words[3], round // 6)).zfill(8)
            roundKey[1] = self.xor(words[1], roundKey[0]).zfill(8)
            roundKey[2] = self.xor(words[2], roundKey[1]).zfill(8)
            roundKey[3] = self.xor(words[3], roundKey[2]).zfill(8)

        return roundKey

    # KEY ADDITION
    def keyAddition(self, state, key):
        stateString = ''
        keyString = ''
        newState = []

        for byte in state:
            stateString += byte

        for word in key:
            keyString += word
            # print(word)
        stateString = self.xor(stateString, keyString).zfill(len(stateString))
        # print(stateString)
        while stateString:
            newState.append(stateString[:2])
            stateString = stateString[2:]

        return newState

    # BYTE SUBSTITUTION
    def byteSubstitution(self, state):
        newState = []

        for byte in state:
            out = int("0x{}".format(byte), 0)
            out = str(hex(self.sBox[out]))[2:]

            if len(out) < 2:
                out = "0" + out

            newState.append(out)

        return newState

    # SHIFT ROWS
    def shiftRows(self, state):
        newState = [None for k in range(16)]

        # No Shift
        newState[0], newState[4], newState[8], newState[12] = state[0], state[4], state[8], state[12]

        # First shift. Shift by one
        newState[1], newState[5], newState[9], newState[13] = state[5], state[9], state[13], state[1]

        # Second shift. Shift by two
        newState[2], newState[6], newState[10], newState[14] = state[10], state[14], state[2], state[6]

        # Third shift. Shift by three
        newState[3], newState[7], newState[11], newState[15] = state[15], state[3], state[7], state[11]
        # print(newState)
        return newState

    # MIX COLUMNS
    def mixColumn(self, subState):
        newSubState = []
        currentMultRound = 0

        while len(newSubState) < 4:
            out = int()

            if currentMultRound == 0:
                one = self.GaloisF2[int("0x{}".format(subState[0]), 0)]
                two = self.GaloisF3[int("0x{}".format(subState[1]), 0)]
                three = int("0x{}".format(subState[2]), 0)
                four = int("0x{}".format(subState[3]), 0)

                out = one ^ two ^ three ^ four
                # print("0", subState[0],subState[1],subState[2],four,out)
            elif currentMultRound == 1:
                one = int("0x{}".format(subState[0]), 0)
                two = self.GaloisF2[int("0x{}".format(subState[1]), 0)]
                three = self.GaloisF3[int("0x{}".format(subState[2]), 0)]
                four = int("0x{}".format(subState[3]), 0)

                out = one ^ two ^ three ^ four
                # print("1", one, two, three, four, out)
            elif currentMultRound == 2:
                one = int("0x{}".format(subState[0]), 0)
                two = int("0x{}".format(subState[1]), 0)
                three = self.GaloisF2[int("0x{}".format(subState[2]), 0)]
                four = self.GaloisF3[int("0x{}".format(subState[3]), 0)]

                out = one ^ two ^ three ^ four
                # print("2", one, two, three, four, out)
            else:
                one = self.GaloisF3[int("0x{}".format(subState[0]), 0)]
                two = int("0x{}".format(subState[1]), 0)
                three = int("0x{}".format(subState[2]), 0)
                four = self.GaloisF2[int("0x{}".format(subState[3]), 0)]

                out = one ^ two ^ three ^ four
                # print("3", one, two, three, four, out)
            out = str(hex(out))[2:]
            if len(out) < 2:
                out = "0" + out

            newSubState.append(out)
            currentMultRound += 1

        return newSubState

    def mixColumns(self, state):
        subState = []
        while state:
            subState += self.mixColumn(state[:4])
            state = state[4:]

        return subState
    def format_state(self, state):
        state_str = ""
        for byte in state:
            state_str += byte + " "
        return state_str.upper()   
    
    def encrypt12round(self, plaintext, key):
        # encrypting TEN ROUNDS
        get_all_key=[]
        #For first key generation.. Because it added automatically
        # print(0,textwrap.wrap(key, 8))
        get_all_key.append(key)
        state = self.initSteMat(plaintext)
        print("=============INITIAL ROUND===============")
        print("Initial Matrix: ")
        # my_matrix=[x.upper() for x in state]
        my_matrix = np.array(state).reshape((4, 4)).T
        print(my_matrix)

        print("Initial Key:")
        rklist = textwrap.wrap(key, 2)
        my_matrix = np.array(rklist).reshape((4, 4)).T
        print(my_matrix)

        # Initial Key Addition Layer
        state = self.keyAddition(state, key)
        print(" After first key addition: ")
        my_matrix = np.array(state).reshape((4, 4)).T
        print(my_matrix)
        initialise=1

        for round in range(12):

            """print("=============ROUND-",initialise,"===============")
            print("Round: ", initialise)
            print("State Matrix")

            print(my_matrix)"""



            state = self.byteSubstitution(state)

            #print(" After Byte Subs:")
            my_matrix = np.array(state).reshape((4, 4)).T
            #print(my_matrix)




            state = self.shiftRows(state)

            #print(" After Row Shifts:")
            my_matrix = np.array(state).reshape((4, 4)).T
            #print(my_matrix)

            if round < 9:
                state = self.mixColumns(state)
                #print(" After Mix Column:")
                my_matrix = np.array(state).reshape((4, 4)).T
                #print(my_matrix)

            newKey = ""
            for byte in key:
                newKey += byte


            key = newKey

            key = self.keySchedule(key, round)

            #print(" Round ", initialise, "Key:")
            r_key=''.join(map(str, key))
            rklist= textwrap.wrap(r_key, 2)
            my_matrix = np.array(rklist).reshape((4, 4)).T
            #print(my_matrix)

            state = self.keyAddition(state, key)

            #print(" After Key Addition:")
            my_matrix = np.array(state).reshape((4, 4)).T
            #print(my_matrix)
            # print(round+1,key)

            pkey=''.join(map(str, key))
            get_all_key.append(pkey)

            # print(get_all_key)
            initialise +=1
            self.round_outputs.append(self.format_state(state))

        return state,get_all_key
        
     
        



if __name__ == "__main__":
    plaintext = "3243F6A8885A308D313198A2E0370734"
    key = "2B7E151628AED2A6ABF7158809CF4F3C"
    AESTest = AesFromScratch()

    [tenRound, allkey] = AESTest.encrypt12round(plaintext, key)

    prettyCipherText = ""
    for byte in tenRound:
        prettyCipherText += byte + " "
        prettyCipherText = prettyCipherText.upper()
    print("\n\nPlaintext:")
    print(plaintext)

    print("\nCipher text:")
    print(prettyCipherText)
    print("\n")

    # Print the output of each round
    for i, output in enumerate(AESTest.round_outputs):
        print("Round", i + 1, "Output:")
        print(output)

   

    print("\n\n--------------------------------------First hex value changed------------------------------------------")
    plaintext2 = "4243F6A8885A308D313198A2E0370734"
    AESTest2 = AesFromScratch()

    [tenRound2, allkey2] = AESTest2.encrypt12round(plaintext2, key)

    prettyCipherText2 = ""
    for byte in tenRound2:
        prettyCipherText2 += byte + " "
        prettyCipherText2 = prettyCipherText2.upper()
    print("\n\nPlaintext:")
    print(plaintext2)

    print("\nCipher text:")
    print(prettyCipherText2)
    print("\n")

    # Print the output of each round
    for i, output2 in enumerate(AESTest2.round_outputs):
        print("Round", i + 1, "Output:")
        print(output2)
        
    print("\n\n------------------------Avalanche Effect--------------------------------\n")    
    #total_changed_bits = 0

    # Compare the round outputs of AESTest and AESTest2
    for i in range(12):  # Assuming there are 10 rounds
        round_output_A = AESTest.round_outputs[i].replace(" ", "")  # Remove spaces
        round_output_B = AESTest2.round_outputs[i].replace(" ", "")  # Remove spaces

        # Convert hexadecimal strings to binary strings
        binary_A = bin(int(round_output_A, 16))[2:].zfill(len(round_output_A) * 4)
        binary_B = bin(int(round_output_B, 16))[2:].zfill(len(round_output_B) * 4)

        # Calculate the number of differing bits
        changed_bits = sum(bit_A != bit_B for bit_A, bit_B in zip(binary_A, binary_B))

        print("Round", i + 1, "Changed bits:", changed_bits)
        #total_changed_bits += changed_bits

    #print("\nTotal changed bits:", total_changed_bits) 
    
    
    print("\n\n******************************************************------Question 2-------*********************************************************\n\n")
    print("\n----------------------------Last hex value changed----------------------------------")
    plaintext3 = "3243F6A8885A308D313198A2E0370735"
    AESTest3 = AesFromScratch()

    [tenRound3, allkey3] = AESTest3.encrypt12round(plaintext3, key)

    prettyCipherText3 = ""
    for byte in tenRound3:
        prettyCipherText3 += byte + " "
        prettyCipherText3 = prettyCipherText3.upper()
    print("\n\nPlaintext:")
    print(plaintext3)

    print("\nCipher text:")
    print(prettyCipherText3)
    print("\n")

    # Print the output of each round
    for i, output3 in enumerate(AESTest3.round_outputs):
        print("Round", i + 1, "Output:")
        print(output3)
    
    print("\n\n------------------------Avalanche Effect--------------------------------\n")    
    #total_changed_bits3 = 0

    # Compare the round outputs of AESTest and AESTest3
    for i in range(12):  # Assuming there are 10 rounds
        round_output_A3 = AESTest.round_outputs[i].replace(" ", "")  # Remove spaces
        round_output_B3 = AESTest3.round_outputs[i].replace(" ", "")  # Remove spaces

        # Convert hexadecimal strings to binary strings
        binary_A3 = bin(int(round_output_A3, 16))[2:].zfill(len(round_output_A3) * 4)
        binary_B3 = bin(int(round_output_B3, 16))[2:].zfill(len(round_output_B3) * 4)

        # Calculate the number of differing bits
        changed_bits3 = sum(bit_A3 != bit_B3 for bit_A3, bit_B3 in zip(binary_A3, binary_B3))

        print("Round", i + 1, "Changed bits:", changed_bits3)
        #total_changed_bits3 += changed_bits3

    #print("\nTotal changed bits:", total_changed_bits3) 
    

    
    
    
